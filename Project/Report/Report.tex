...
1 -Este trigger permite manter um número de amigos limitados na rede social, ou seja, quando há uma inserção na tabela Friendship se o utilizador já tiver o número máximo de amigos a inserção é parada.

2 - Este trigger faz com que quando um utilizador pressiona um botão a dizer que vai ou foi a um certo evento(há uma inserção na tabela EventParticipant) este faz um post no perfil do utilizador. O conteúdo deste post pode ser "I went to (event Name)" se o evento já tiver acontecido e "I will participate in (event name)" se ainda for acontecer.

3 - Este trigger faz com que quando um utilizador é convidado para ser administrador de um grupo ao qual não pertence este é automaticamente adicionado e o administrador atual perde o cargo e passa a ser membro.